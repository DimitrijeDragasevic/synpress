...
class SeedPage extends BasePage {
  ...
  async initialize() {
    if(this.basePage == null || this.basePage == undefined) {
  ...
  async createPage() {
      ...
      await this.basePage.getByText('Import from seed phrase').click();
  ...
  async fillSeedForm(walletName, password, seed = process.env.SEED_PHRASE) {
    ...
    await expect(
      await this.page.getByTestId('DoneAllIcon'),
    ).toBeVisible();
    await expect(
  ...
  async verifyFirstWalletAdded() {
    ...
    await expect(
      await this.page.getByText('Manage Wallets'),
    ).toBeVisible();
    await expect(
      await this.page.getByRole('button', {
    ...
    await expect(
      await this.page.getByText('Add a wallet'),
    ).toBeVisible();
